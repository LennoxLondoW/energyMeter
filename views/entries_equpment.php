<?php
session_start();
require_once '../controlers/entries_equpmentControler.php';
  ...
  <div style="min-height:70vh; padding:50px 20px;">
    ...
    <form method="post" class="ajax" enctype="multipart/form-data" action="<?php echo base_path . str_replace(".php", "", basename($_SERVER['PHP_SELF']));  ?>">
      ...
      <div class="form-group">
        ...
        <select class="inputs" id="meter_serial_number" name="meter_json" required>
          <option value="" selected disabled><?php echo $label1; ?></option>
          <?php
          foreach ($equipment_data as $value) {
            echo "<option value='" . json_encode($value) . "'>" . $value['equipment_name'] . "</option>";
          }
          ?>
        </select>
      </div>
    ...
    ?>
      ...
      <form method="post" class="bulk_equpment" id="bulk_equpment" enctype="multipart/form-data" action="<?php echo base_path . str_replace(".php", "", basename($_SERVER['PHP_SELF']));  ?>">
        <div class="form-group" id="bulk_equpment_result" style="max-height: 150px !important; overflow-y:auto;">
        ...
        <div class="form-group">
          <select class="inputs" id="meter_serial_number2" name="meter_json" required>
            <option value="" selected disabled><?php echo $label1; ?></option>
            <?php
            foreach ($equipment_data as $value) {
              echo "<option value='" . json_encode($value) . "'>" . $value['equipment_name'] . "</option>";
