<?php
session_start();
require_once '../controlers/analytics_equipmentControler.php';
...
<div class="container ck">
  ...
  <div style="min-height:70vh; padding:50px 20px;" id="graph_parent">
    <form class="ajax inline center" id="graph_form" method="post" action="<?php echo base_path . str_replace(".php", "", basename($_SERVER['PHP_SELF']));  ?>">
      ...
      <select class="inputs" name="eqipment_name" required>
        <?php
        foreach ($equipment_data as $value) {
          echo "<option value='" . $value['equipment_name'] . "'>" . $value['equipment_name'] . "</option>";
        }
        ?>
      </select>
